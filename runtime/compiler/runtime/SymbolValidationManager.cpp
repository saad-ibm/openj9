    ...
    , _loadersOkForWellKnownClasses(_region)
{
    ...
    for (int32_t i = 4; i <= 11; i++) {
        ...
        // redundant.
        _alreadyGeneratedRecords.insert(new (_region) ArrayClassFromComponentClassRecord(arrayClass, component));
    }

    // Ensure that the count of classes not worth remembering is defined correctly
    static_assert(SYSTEM_CLASSES_NOT_WORTH_REMEMBERING_COUNT
            == sizeof(_systemClassesNotWorthRemembering) / sizeof(_systemClassesNotWorthRemembering[0]),
        "SYSTEM_CLASSES_NOT_WORTH_REMEMBERING_COUNT doesn't match the size of _systemClassesNotWorthRemembering");
}

#if defined(J9VM_OPT_JITSERVER)
void TR::SymbolValidationManager::populateSystemClassesNotWorthRemembering(ClientSessionData *clientData)
{
    ...
    for (int i = 0; i < SYSTEM_CLASSES_NOT_WORTH_REMEMBERING_COUNT; ++i) {
        ...
        classesNotWorthRemembering[i]
            = { classNotWorthRemembering->_className, NULL, classNotWorthRemembering->_checkIsSuperClass };
    }
}
#endif

void TR::SymbolValidationManager::defineGuaranteedID(void *value, TR::SymbolType type)
{
...
bool TR::SymbolValidationManager::isClassWorthRemembering(TR_OpaqueClassBlock *clazz)
{
    ...
    for (int i = 0; worthRemembering && i < SYSTEM_CLASSES_NOT_WORTH_REMEMBERING_COUNT; i++) {
        ...
        if (!systemClassNotWorthRemembering->_clazz) {
            systemClassNotWorthRemembering->_clazz
                = _fej9->getSystemClassFromClassName(systemClassNotWorthRemembering->_className,
        ...
        if (systemClassNotWorthRemembering->_checkIsSuperClass) {
            if (systemClassNotWorthRemembering->_clazz
                ...
                logprintf(_comp->getOption(TR_TraceRelocatableDataCG), _comp->log(),
                    "isClassWorthRemembering: clazz %p is or inherits from %s (%p)\n", clazz,
                    systemClassNotWorthRemembering->_className, systemClassNotWorthRemembering->_clazz);
...
void TR::SymbolValidationManager::populateWellKnownClasses()
{
#define REQUIRED_WELL_KNOWN_CLASS_COUNT 0
    ...
    static_assert(sizeof(names) / sizeof(names[0]) == WELL_KNOWN_CLASS_COUNT, "wrong number of well-known class names");

    static_assert(CHAR_BIT * sizeof(includedClasses) >= WELL_KNOWN_CLASS_COUNT,
    ...
    for (int i = 0; i < WELL_KNOWN_CLASS_COUNT; i++) {
        ...
        if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET == chainOffset) {
            ...
            SVM_ASSERT_NONFATAL(i >= REQUIRED_WELL_KNOWN_CLASS_COUNT, "failed to remember required class %s\n", name);
            continue;
        }
    ...
    *classCount = _wellKnownClasses.size();
    ...
    if (clientData) {
        ...
        if (aotCacheStore && clientData->useServerOffsets(_comp->getStream())) {
            // getWellKnownClassesRecord expects the number of well-known classes, not the number of elements in the
            // object
            _aotCacheWellKnownClassesRecord
                = clientData->getWellKnownClassesRecord(classChainRecords, _wellKnownClasses.size(), includedClasses);
            // TODO: I think _wellKnownClassChainOffsets can remain NULL
            return;
        }

        // Otherwise check the cache in the client session first
        _wellKnownClassChainOffsets = clientData->getCachedWellKnownClassChainOffsets(includedClasses,
            _wellKnownClasses.size(), classChainOffsets + 1, _aotCacheWellKnownClassesRecord);
        if (_wellKnownClassChainOffsets)
            return;
    }
#endif /* defined(J9VM_OPT_JITSERVER) */

    _wellKnownClassChainOffsets = _fej9->sharedCache()->storeWellKnownClasses(_vmThread, classChainOffsets,
        1 + _wellKnownClasses.size(), includedClasses);
    ...
    if (_wellKnownClassChainOffsets && clientData) {
        ...
        clientData->cacheWellKnownClassChainOffsets(includedClasses, _wellKnownClasses.size(), classChainOffsets + 1,
            _wellKnownClassChainOffsets, (aotCacheStore && !missingClassChainRecords) ? classChainRecords : NULL,
            _aotCacheWellKnownClassesRecord);
    }
#endif /* defined(J9VM_OPT_JITSERVER) */

    SVM_ASSERT_NONFATAL(_wellKnownClassChainOffsets != NULL, "Failed to store well-known classes' class chains");

#undef REQUIRED_WELL_KNOWN_CLASS_COUNT
}

bool TR::SymbolValidationManager::validateWellKnownClasses(const uintptr_t *wellKnownClassChainOffsets)
{
    ...
    for (int i = 1; i <= classCount; i++) {
        ...
        uintptr_t *classChain
            = reinterpret_cast<uintptr_t *>(_fej9->sharedCache()->pointerFromOffsetInSharedCache(classChainOffset));
        ...
        TR_OpaqueClassBlock *clazz = _fej9->getSystemClassFromClassName(
            reinterpret_cast<const char *>(J9UTF8_DATA(className)), J9UTF8_LENGTH(className));
...
bool TR::SymbolValidationManager::isWellKnownClass(TR_OpaqueClassBlock *clazz)
{
    auto end = _wellKnownClasses.end();
    return std::find(_wellKnownClasses.begin(), end, clazz) != end;
}

bool TR::SymbolValidationManager::classCanSeeWellKnownClasses(TR_OpaqueClassBlock *beholder)
{
    ...
    J9ClassLoader *loader = reinterpret_cast<J9ClassLoader *>(_fej9->getClassLoader(beholder));
    {
...
bool TR::SymbolValidationManager::isDefinedID(uint16_t id)
{
    return id < _symbolToValueTable.size() && _symbolToValueTable[id]._hasValue;
}

void TR::SymbolValidationManager::setValueOfSymbolID(uint16_t id, void *value, TR::SymbolType type)
{
...
uint16_t TR::SymbolValidationManager::getNewSymbolID()
{
...
TR_OpaqueClassBlock *TR::SymbolValidationManager::getClassFromID(uint16_t id, Presence presence)
{
    return static_cast<TR_OpaqueClassBlock *>(getValueFromSymbolID(id, TR::SymbolType::typeClass, presence));
}

J9Class *TR::SymbolValidationManager::getJ9ClassFromID(uint16_t id, Presence presence)
{
    return static_cast<J9Class *>(getValueFromSymbolID(id, TR::SymbolType::typeClass, presence));
}

TR_OpaqueMethodBlock *TR::SymbolValidationManager::getMethodFromID(uint16_t id, Presence presence)
{
    return static_cast<TR_OpaqueMethodBlock *>(getValueFromSymbolID(id, TR::SymbolType::typeMethod, presence));
}

J9Method *TR::SymbolValidationManager::getJ9MethodFromID(uint16_t id, Presence presence)
{
    return static_cast<J9Method *>(getValueFromSymbolID(id, TR::SymbolType::typeMethod, presence));
}

uint16_t TR::SymbolValidationManager::tryGetSymbolIDFromValue(void *value)
{
...
TR_OpaqueClassBlock *TR::SymbolValidationManager::getBaseComponentClass(TR_OpaqueClassBlock *clazz, int32_t &numDims)
{
    numDims = 0;
    if (!clazz)
        return NULL;
...
bool TR::SymbolValidationManager::abandonRecord(TR::SymbolValidationRecord *record)
{
    _region.deallocate(record);
    return inHeuristicRegion();
}

bool TR::SymbolValidationManager::recordExists(TR::SymbolValidationRecord *record)
{
    return _alreadyGeneratedRecords.find(record) != _alreadyGeneratedRecords.end();
}

bool TR::SymbolValidationManager::anyClassFromCPRecordExists(TR_OpaqueClassBlock *clazz, TR_OpaqueClassBlock *beholder)
{
    ClassFromAnyCPIndex pair(clazz, beholder);
    return _classesFromAnyCPIndex.find(pair) != _classesFromAnyCPIndex.end();
}

void TR::SymbolValidationManager::appendNewRecord(void *value, TR::SymbolValidationRecord *record)
{
...
void TR::SymbolValidationManager::appendRecordIfNew(void *value, TR::SymbolValidationRecord *record)
{
...
bool TR::SymbolValidationManager::getClassChainInfo(TR_OpaqueClassBlock *clazz, TR::SymbolValidationRecord *record,
    ClassChainInfo &info)
{
...
void TR::SymbolValidationManager::appendClassChainInfoRecords(TR_OpaqueClassBlock *clazz, const ClassChainInfo &info)
{
    ...
    for (int i = 0; i < info._arrayDims; i++) {
        ...
        appendRecordIfNew(component, new (_region) ArrayClassFromComponentClassRecord(clazz, component));
        clazz = component;
    }

    // If necessary, remember to validate the class chain of the base component type
    if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET != info._baseComponentClassChainOffset) {
        appendNewRecord(info._baseComponent,
            new (_region) ClassChainRecord(info._baseComponent, info._baseComponentClassChainOffset
#if defined(J9VM_OPT_JITSERVER)
                ,
...
bool TR::SymbolValidationManager::addVanillaRecord(void *value, TR::SymbolValidationRecord *record)
{
...
bool TR::SymbolValidationManager::addClassRecord(TR_OpaqueClassBlock *clazz, TR::ClassValidationRecord *record)
{
...
bool TR::SymbolValidationManager::addClassRecordWithChain(TR::ClassValidationRecordWithChain *record)
{
...
void TR::SymbolValidationManager::addMultipleArrayRecords(TR_OpaqueClassBlock *component, int arrayDims)
{
    for (int i = 0; i < arrayDims; i++) {
        TR_OpaqueClassBlock *array = _fej9->getArrayClassFromComponentClass(component);
        appendRecordIfNew(array, new (_region) ArrayClassFromComponentClassRecord(array, component));
        component = array;
    }
}

bool TR::SymbolValidationManager::addMethodRecord(TR::MethodValidationRecord *record)
{
    if (shouldNotDefineSymbol(record->_method) || !isClassWorthRemembering(_fej9->getClassOfMethod(record->_method))) {
        return abandonRecord(record);
    }
...
bool TR::SymbolValidationManager::skipFieldRefClassRecord(TR_OpaqueClassBlock *definingClass,
    TR_OpaqueClassBlock *beholder, uint32_t cpIndex)
{
    ...
    if (definingClass == beholder || isWellKnownClass(definingClass)) {
        ...
        if (classRefLen == definingClassLen && !memcmp(classRefName, definingClassName, classRefLen)) {
...
bool TR::SymbolValidationManager::addClassByNameRecord(TR_OpaqueClassBlock *clazz, TR_OpaqueClassBlock *beholder)
{
...
bool TR::SymbolValidationManager::addProfiledClassRecord(TR_OpaqueClassBlock *clazz)
{
...
bool TR::SymbolValidationManager::addClassFromCPRecord(TR_OpaqueClassBlock *clazz,
    J9ConstantPool *constantPoolOfBeholder, uint32_t cpIndex)
{
...
bool TR::SymbolValidationManager::addDefiningClassFromCPRecord(TR_OpaqueClassBlock *clazz,
    J9ConstantPool *constantPoolOfBeholder, uint32_t cpIndex, bool isStatic)
{
    TR_OpaqueClassBlock *beholder = _fej9->getClassFromCP(constantPoolOfBeholder);
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addStaticClassFromCPRecord(TR_OpaqueClassBlock *clazz,
    J9ConstantPool *constantPoolOfBeholder, uint32_t cpIndex)
{
    TR_OpaqueClassBlock *beholder = _fej9->getClassFromCP(constantPoolOfBeholder);
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addArrayClassFromComponentClassRecord(TR_OpaqueClassBlock *arrayClass,
...
bool TR::SymbolValidationManager::addSuperClassFromClassRecord(TR_OpaqueClassBlock *superClass,
    TR_OpaqueClassBlock *childClass)
{
...
bool TR::SymbolValidationManager::addClassInstanceOfClassRecord(TR_OpaqueClassBlock *classOne,
    TR_OpaqueClassBlock *classTwo, bool objectTypeIsFixed, bool castTypeIsFixed, bool isInstanceOf)
{
    ...
    // Not using addClassRecord() because this doesn't define a class symbol. We
    // can pass either class as the symbol because neither will get a fresh ID
    return addVanillaRecord(classOne,
        new (_region) ClassInstanceOfClassRecord(classOne, classTwo, objectTypeIsFixed, castTypeIsFixed, isInstanceOf));
}

bool TR::SymbolValidationManager::addSystemClassByNameRecord(TR_OpaqueClassBlock *systemClass)
{
...
bool TR::SymbolValidationManager::addClassFromITableIndexCPRecord(TR_OpaqueClassBlock *clazz,
    J9ConstantPool *constantPoolOfBeholder, int32_t cpIndex)
{
    TR_OpaqueClassBlock *beholder = _fej9->getClassFromCP(constantPoolOfBeholder);
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addDeclaringClassFromFieldOrStaticRecord(TR_OpaqueClassBlock *clazz,
    J9ConstantPool *constantPoolOfBeholder, int32_t cpIndex)
{
    TR_OpaqueClassBlock *beholder = _fej9->getClassFromCP(constantPoolOfBeholder);
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addConcreteSubClassFromClassRecord(TR_OpaqueClassBlock *childClass,
...
bool TR::SymbolValidationManager::addMethodFromClassRecord(TR_OpaqueMethodBlock *method, TR_OpaqueClassBlock *beholder,
    uint32_t index)
{
    ...
    if (index == static_cast<uint32_t>(-1)) {
        ...
        SVM_ASSERT(index < numMethods, "Method %p not found in class %p", method, beholder);
    }
...
bool TR::SymbolValidationManager::addStaticMethodFromCPRecord(TR_OpaqueMethodBlock *method, J9ConstantPool *cp,
    int32_t cpIndex)
{
...
bool TR::SymbolValidationManager::addSpecialMethodFromCPRecord(TR_OpaqueMethodBlock *method, J9ConstantPool *cp,
    int32_t cpIndex)
{
    TR_OpaqueClassBlock *beholder = _fej9->getClassFromCP(cp);
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addVirtualMethodFromCPRecord(TR_OpaqueMethodBlock *method, J9ConstantPool *cp,
    int32_t cpIndex)
{
    TR_OpaqueClassBlock *beholder = _fej9->getClassFromCP(cp);
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addVirtualMethodFromOffsetRecord(TR_OpaqueMethodBlock *method,
    TR_OpaqueClassBlock *beholder, int32_t virtualCallOffset, bool ignoreRtResolve)
{
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
    ...
    return addMethodRecord(
        new (_region) VirtualMethodFromOffsetRecord(method, beholder, virtualCallOffset, ignoreRtResolve));
}

bool TR::SymbolValidationManager::addInterfaceMethodFromCPRecord(TR_OpaqueMethodBlock *method,
    TR_OpaqueClassBlock *beholder, TR_OpaqueClassBlock *lookup, int32_t cpIndex)
{
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addImproperInterfaceMethodFromCPRecord(TR_OpaqueMethodBlock *method,
    J9ConstantPool *cp, int32_t cpIndex)
{
    TR_OpaqueClassBlock *beholder = _fej9->getClassFromCP(cp);
    SVM_ASSERT_ALREADY_VALIDATED(this, beholder);
...
bool TR::SymbolValidationManager::addMethodFromClassAndSignatureRecord(TR_OpaqueMethodBlock *method,
    TR_OpaqueClassBlock *lookupClass, TR_OpaqueClassBlock *beholder)
{
...
bool TR::SymbolValidationManager::addMethodFromSingleImplementerRecord(TR_OpaqueMethodBlock *method,
    TR_OpaqueClassBlock *thisClass, int32_t cpIndexOrVftSlot, TR_OpaqueMethodBlock *callerMethod,
    TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
    ...
    return addMethodRecord(new (_region) MethodFromSingleImplementer(method, thisClass, cpIndexOrVftSlot, callerMethod,
        useGetResolvedInterfaceMethod));
}

bool TR::SymbolValidationManager::addMethodFromSingleInterfaceImplementerRecord(TR_OpaqueMethodBlock *method,
    TR_OpaqueClassBlock *thisClass, int32_t cpIndex, TR_OpaqueMethodBlock *callerMethod)
{
    SVM_ASSERT_ALREADY_VALIDATED(this, thisClass);
    SVM_ASSERT_ALREADY_VALIDATED(this, callerMethod);
    return addMethodRecord(
        new (_region) MethodFromSingleInterfaceImplementer(method, thisClass, cpIndex, callerMethod));
}

bool TR::SymbolValidationManager::addMethodFromSingleAbstractImplementerRecord(TR_OpaqueMethodBlock *method,
    TR_OpaqueClassBlock *thisClass, int32_t vftSlot, TR_OpaqueMethodBlock *callerMethod)
{
    SVM_ASSERT_ALREADY_VALIDATED(this, thisClass);
    SVM_ASSERT_ALREADY_VALIDATED(this, callerMethod);
...
bool TR::SymbolValidationManager::addDynamicMethodFromCallsiteIndex(TR_OpaqueMethodBlock *method,
    TR_OpaqueMethodBlock *caller, int32_t callsiteIndex, bool appendixObjectNull)
{
    SVM_ASSERT_ALREADY_VALIDATED(this, caller);
    return addMethodRecord(
        new (_region) DynamicMethodFromCallsiteIndexRecord(method, caller, callsiteIndex, appendixObjectNull));
}

bool TR::SymbolValidationManager::addHandleMethodFromCPIndex(TR_OpaqueMethodBlock *method, TR_OpaqueMethodBlock *caller,
    int32_t cpIndex, bool appendixObjectNull)
{
    SVM_ASSERT_ALREADY_VALIDATED(this, caller);
...
bool TR::SymbolValidationManager::addClassInfoIsInitializedRecord(TR_OpaqueClassBlock *clazz, bool isInitialized)
{
...
void TR::SymbolValidationManager::addJ2IThunkFromMethodRecord(void *thunk, TR_OpaqueMethodBlock *method)
{
    ...
    TR::SymbolValidationRecord *record = new (_region) J2IThunkFromMethodRecord(thunk, method);

    SVM_ASSERT(!recordExists(record),
...
bool TR::SymbolValidationManager::addIsClassVisibleRecord(TR_OpaqueClassBlock *sourceClass,
    TR_OpaqueClassBlock *destClass, bool isVisible)
{
    ...
    if (sourceClass == destClass || _fej9->isJavaLangObject(destClass))
        return true;
    ...
    if (entry == NULL || !entry->_hasValue) {
        if (_seenValuesSet.find(validValue) == _seenValuesSet.end()) {
            ...
            if (type == TR::SymbolType::typeClass) {
                valid = classCanSeeWellKnownClasses(reinterpret_cast<TR_OpaqueClassBlock *>(validValue));
            }
        ...
        valid = validValue == entry->_value && (validValue == NULL || entry->_type == type);
    }

    return valid;
}

bool TR::SymbolValidationManager::validateSymbol(uint16_t idToBeValidated, TR_OpaqueClassBlock *clazz)
{
    return validateSymbol(idToBeValidated, clazz, TR::SymbolType::typeClass);
}

bool TR::SymbolValidationManager::validateSymbol(uint16_t idToBeValidated, J9Class *clazz)
{
    return validateSymbol(idToBeValidated, clazz, TR::SymbolType::typeClass);
}

bool TR::SymbolValidationManager::validateSymbol(uint16_t methodID, uint16_t definingClassID,
    TR_OpaqueMethodBlock *method)
{
    return validateSymbol(methodID, definingClassID, reinterpret_cast<J9Method *>(method));
}

bool TR::SymbolValidationManager::validateSymbol(uint16_t methodID, uint16_t definingClassID, J9Method *method)
{
...
bool TR::SymbolValidationManager::validateClassByNameRecord(uint16_t classID, uint16_t beholderID,
    uintptr_t *classChain)
{
    ...
    return validateSymbol(classID, clazz) && _fej9->sharedCache()->classMatchesCachedVersion(clazz, classChain);
}

bool TR::SymbolValidationManager::validateProfiledClassRecord(uint16_t classID, void *classChainIdentifyingLoader,
    void *classChainForClassBeingValidated)
{
    TR_OpaqueClassBlock *clazz = NULL;
    J9ClassLoader *classLoader
        = (J9ClassLoader *)_fej9->sharedCache()->lookupClassLoaderAssociatedWithClassChain(classChainIdentifyingLoader);
    if (classLoader) {
    ...
    if (!clazz) {
        if (auto dependencyTable = _comp->getPersistentInfo()->getAOTDependencyTable())
            clazz = (TR_OpaqueClassBlock *)dependencyTable->findCandidateWithChainAndLoader(_comp,
                (uintptr_t *)classChainForClassBeingValidated, classChainIdentifyingLoader);
    }

    return validateSymbol(classID, clazz);
}

bool TR::SymbolValidationManager::validateClassFromCPRecord(uint16_t classID, uint16_t beholderID, uint32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
...
bool TR::SymbolValidationManager::validateDefiningClassFromCPRecord(uint16_t classID, uint16_t beholderID,
    uint32_t cpIndex, bool isStatic)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
    return validateSymbol(classID,
        TR_ResolvedJ9Method::definingClassFromCPFieldRef(_comp, beholderCP, cpIndex, isStatic));
}

bool TR::SymbolValidationManager::validateStaticClassFromCPRecord(uint16_t classID, uint16_t beholderID,
    uint32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
    ...
    if (cpIndex != -1) {
        ...
        if (!clazz) {
            ...
            _vmThread->javaVM->internalVMFunctions->resolveStaticFieldRef(_fej9->vmThread(), NULL, beholderCP, cpIndex,
                J9_RESOLVE_FLAG_JIT_COMPILE_TIME, NULL);
            clazz = TR_ResolvedJ9Method::getClassOfStaticFromCP(_fej9, beholderCP, cpIndex);
        }
    }
...
bool TR::SymbolValidationManager::validateArrayClassFromComponentClassRecord(uint16_t arrayClassID,
...
bool TR::SymbolValidationManager::validateSuperClassFromClassRecord(uint16_t superClassID, uint16_t childClassID)
{
...
bool TR::SymbolValidationManager::validateClassInstanceOfClassRecord(uint16_t classOneID, uint16_t classTwoID,
    bool objectTypeIsFixed, bool castTypeIsFixed, bool wasInstanceOf)
{
...
bool TR::SymbolValidationManager::validateSystemClassByNameRecord(uint16_t systemClassID, uintptr_t *classChain)
{
    J9ROMClass *romClass = _fej9->sharedCache()->startingROMClassOfClassChain(classChain);
    J9UTF8 *className = J9ROMCLASS_CLASSNAME(romClass);
    TR_OpaqueClassBlock *systemClassByName = _fej9->getSystemClassFromClassName(
        reinterpret_cast<const char *>(J9UTF8_DATA(className)), J9UTF8_LENGTH(className));
...
bool TR::SymbolValidationManager::validateClassFromITableIndexCPRecord(uint16_t classID, uint16_t beholderID,
    uint32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
    ...
    return validateSymbol(classID,
        TR_ResolvedJ9Method::getInterfaceITableIndexFromCP(_fej9, beholderCP, cpIndex, &pITableIndex));
}

bool TR::SymbolValidationManager::validateDeclaringClassFromFieldOrStaticRecord(uint16_t definingClassID,
    uint16_t beholderID, int32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    ...
    J9Class *cpClass
        = (J9Class *)TR_ResolvedJ9Method::getClassFromCP(_fej9, beholderCP, _comp, classCPIndexOfFieldOrStatic);

    if (cpClass) {
        TR::VMAccessCriticalSection getDeclaringClassFromFieldOrStatic(_fej9);

        int32_t fieldLen = 0;
        char *field = cpIndex >= 0
            ? utf8Data(J9ROMNAMEANDSIGNATURE_NAME(J9ROMFIELDREF_NAMEANDSIGNATURE(&romCPBase[cpIndex])), fieldLen)
            : 0;

        int32_t sigLen = 0;
        char *sig = cpIndex >= 0
            ? utf8Data(J9ROMNAMEANDSIGNATURE_SIGNATURE(J9ROMFIELDREF_NAMEANDSIGNATURE(&romCPBase[cpIndex])), sigLen)
            : 0;

        _vmThread->javaVM->internalVMFunctions->instanceFieldOffset(_vmThread, cpClass, (U_8 *)field, fieldLen,
            (U_8 *)sig, sigLen, &definingClass, NULL, J9_LOOK_NO_JAVA);
    } else {
        return false;
    }
...
bool TR::SymbolValidationManager::validateConcreteSubClassFromClassRecord(uint16_t childClassID, uint16_t superClassID)
{
...
bool TR::SymbolValidationManager::validateClassChainRecord(uint16_t classID, void *classChain)
{
...
bool TR::SymbolValidationManager::validateMethodFromClassRecord(uint16_t methodID, uint16_t beholderID, uint32_t index)
{
...
bool TR::SymbolValidationManager::validateStaticMethodFromCPRecord(uint16_t methodID, uint16_t definingClassID,
    uint16_t beholderID, int32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
...
bool TR::SymbolValidationManager::validateSpecialMethodFromCPRecord(uint16_t methodID, uint16_t definingClassID,
    uint16_t beholderID, int32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
...
bool TR::SymbolValidationManager::validateVirtualMethodFromCPRecord(uint16_t methodID, uint16_t definingClassID,
    uint16_t beholderID, int32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
    ...
    TR_OpaqueMethodBlock *ramMethod
        = TR_ResolvedJ9Method::getVirtualMethod(_fej9, beholderCP, cpIndex, &vTableOffset, &unresolvedInCP);

    return validateSymbol(methodID, definingClassID, ramMethod);
}

bool TR::SymbolValidationManager::validateVirtualMethodFromOffsetRecord(uint16_t methodID, uint16_t definingClassID,
    uint16_t beholderID, int32_t virtualCallOffset, bool ignoreRtResolve)
{
    TR_OpaqueClassBlock *beholder = getClassFromID(beholderID);
...
bool TR::SymbolValidationManager::validateInterfaceMethodFromCPRecord(uint16_t methodID, uint16_t definingClassID,
    uint16_t beholderID, uint16_t lookupID, int32_t cpIndex)
{
...
bool TR::SymbolValidationManager::validateImproperInterfaceMethodFromCPRecord(uint16_t methodID,
    uint16_t definingClassID, uint16_t beholderID, int32_t cpIndex)
{
    J9Class *beholder = getJ9ClassFromID(beholderID);
    J9ConstantPool *beholderCP = J9_CP_FROM_CLASS(beholder);
    J9Method *ramMethod;
...
bool TR::SymbolValidationManager::validateMethodFromClassAndSignatureRecord(uint16_t methodID, uint16_t definingClassID,
    uint16_t lookupClassID, uint16_t beholderID, J9ROMMethod *romMethod)
{
...
bool TR::SymbolValidationManager::validateMethodFromSingleImplementerRecord(uint16_t methodID, uint16_t definingClassID,
    uint16_t thisClassID, int32_t cpIndexOrVftSlot, uint16_t callerMethodID,
    TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
    ...
    TR_ResolvedMethod *calleeResolvedMethod = _chTable->findSingleImplementer(thisClass, cpIndexOrVftSlot,
        callerResolvedMethod, _comp, false, useGetResolvedInterfaceMethod, false);
...
bool TR::SymbolValidationManager::validateMethodFromSingleInterfaceImplementerRecord(uint16_t methodID,
    uint16_t definingClassID, uint16_t thisClassID, int32_t cpIndex, uint16_t callerMethodID)
{
    TR_OpaqueClassBlock *thisClass = getClassFromID(thisClassID);
    TR_OpaqueMethodBlock *callerMethod = getMethodFromID(callerMethodID);
    ...
    TR_ResolvedMethod *calleeResolvedMethod
        = _chTable->findSingleInterfaceImplementer(thisClass, cpIndex, callerResolvedMethod, _comp, false, false);
...
bool TR::SymbolValidationManager::validateMethodFromSingleAbstractImplementerRecord(uint16_t methodID,
    uint16_t definingClassID, uint16_t thisClassID, int32_t vftSlot, uint16_t callerMethodID)
{
    TR_OpaqueClassBlock *thisClass = getClassFromID(thisClassID);
    TR_OpaqueMethodBlock *callerMethod = getMethodFromID(callerMethodID);
    ...
    TR_ResolvedMethod *callerResolvedMethod = _fej9->createResolvedMethod(_trMemory, callerMethod, NULL);
    TR_ResolvedMethod *calleeResolvedMethod
        = _chTable->findSingleAbstractImplementer(thisClass, vftSlot, callerResolvedMethod, _comp, false, false);
...
bool TR::SymbolValidationManager::validateDynamicMethodFromCallsiteIndex(uint16_t methodID, uint16_t callerID,
    int32_t callsiteIndex, bool appendixObjectNull, uint16_t definingClassID, uint32_t methodIndex)
{
    bool valid = false;
    ...
    if (!resolvedCaller->isUnresolvedCallSiteTableEntry(callsiteIndex)) {
        ...
        if (_fej9->isInvokeCacheEntryAnArray(invokeCacheArray)) {
            ...
            TR_OpaqueMethodBlock *targetMethod
                = static_cast<TR_ResolvedJ9Method *>(resolvedCaller)
                      ->getTargetMethodFromMemberName(invokeCacheArray, &invokeCacheAppendixNull);

            TR_OpaqueClassBlock *targetMethodClass = reinterpret_cast<TR_OpaqueClassBlock *>(
                J9_CLASS_FROM_METHOD(reinterpret_cast<J9Method *>(targetMethod)));

            valid = validateSymbol(methodID, definingClassID, targetMethod)
...
bool TR::SymbolValidationManager::validateHandleMethodFromCPIndex(uint16_t methodID, uint16_t callerID, int32_t cpIndex,
    bool appendixObjectNull, uint16_t definingClassID, uint32_t methodIndex)
{
    ...
    if (!resolvedCaller->isUnresolvedMethodTypeTableEntry(cpIndex)) {
        ...
        TR_OpaqueClassBlock *targetMethodClass
            = reinterpret_cast<TR_OpaqueClassBlock *>(J9_CLASS_FROM_METHOD(reinterpret_cast<J9Method *>(targetMethod)));
...
bool TR::SymbolValidationManager::validateClassInfoIsInitializedRecord(uint16_t classID, bool wasInitialized)
{
    TR_OpaqueClassBlock *clazz = getClassFromID(classID);

    bool initialized = false;

    TR_PersistentClassInfo *classInfo = _chTable->findClassInfoAfterLocking(clazz, _comp, true);
...
bool TR::SymbolValidationManager::validateJ2IThunkFromMethodRecord(uint16_t thunkID, void *thunk)
{
    return validateSymbol(thunkID, thunk, TR::SymbolType::typeOpaque);
}

bool TR::SymbolValidationManager::validateIsClassVisibleRecord(uint16_t sourceClassID, uint16_t destClassID,
    bool wasVisible)
{
...
public:
    ...
    LexicalOrder(const LexicalOrder &other)
...
bool TR::ClassByNameRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    ...
    return LexicalOrder::by(_class, rhs->_class).thenBy(_beholder, rhs->_beholder).less();
}
...
bool TR::DefiningClassFromCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::DefiningClassFromCPRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_class, rhs->_class)
        .thenBy(_beholder, rhs->_beholder)
        .thenBy(_cpIndex, rhs->_cpIndex)
        .thenBy(_isStatic, rhs->_isStatic)
        .less();
}
...
bool TR::StaticClassFromCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::ArrayClassFromComponentClassRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::ArrayClassFromComponentClassRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_arrayClass, rhs->_arrayClass).thenBy(_componentClass, rhs->_componentClass).less();
}
...
bool TR::SuperClassFromClassRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::SuperClassFromClassRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_superClass, rhs->_superClass).thenBy(_childClass, rhs->_childClass).less();
}
...
bool TR::ClassInstanceOfClassRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    ...
    return LexicalOrder::by(_classOne, rhs->_classOne)
        ...
        .thenBy(_isInstanceOf, rhs->_isInstanceOf)
        .less();
}
...
bool TR::ClassFromITableIndexCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::DeclaringClassFromFieldOrStaticRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::ConcreteSubClassFromClassRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::ConcreteSubClassFromClassRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_childClass, rhs->_childClass).thenBy(_superClass, rhs->_superClass).less();
}
...
bool TR::MethodFromClassRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::MethodFromClassRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_method, rhs->_method).thenBy(_beholder, rhs->_beholder).thenBy(_index, rhs->_index).less();
}
...
bool TR::StaticMethodFromCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::SpecialMethodFromCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::VirtualMethodFromCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::VirtualMethodFromOffsetRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::VirtualMethodFromOffsetRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_method, rhs->_method)
        .thenBy(_beholder, rhs->_beholder)
        .thenBy(_virtualCallOffset, rhs->_virtualCallOffset)
        .thenBy(_ignoreRtResolve, rhs->_ignoreRtResolve)
        .less();
}
...
bool TR::InterfaceMethodFromCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::MethodFromClassAndSigRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::StackWalkerMaySkipFramesRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::StackWalkerMaySkipFramesRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_method, rhs->_method)
        .thenBy(_methodClass, rhs->_methodClass)
        .thenBy(_skipFrames, rhs->_skipFrames)
        .less();
}
...
bool TR::ClassInfoIsInitialized::isLessThanWithinKind(SymbolValidationRecord *other)
{
    ...
    return LexicalOrder::by(_class, rhs->_class).thenBy(_isInitialized, rhs->_isInitialized).less();
}
...
bool TR::MethodFromSingleImplementer::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::MethodFromSingleInterfaceImplementer::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::MethodFromSingleAbstractImplementer::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::ImproperInterfaceMethodFromCPRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
...
bool TR::J2IThunkFromMethodRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    TR::J2IThunkFromMethodRecord *rhs = downcast(this, other);
    return LexicalOrder::by(_thunk, rhs->_thunk).thenBy(_method, rhs->_method).less();
}
...
bool TR::IsClassVisibleRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    ...
    return LexicalOrder::by(_sourceClass, rhs->_sourceClass)
        ...
        .thenBy(_isVisible, rhs->_isVisible)
        .less();
}
...
bool TR::DynamicMethodFromCallsiteIndexRecord::isLessThanWithinKind(SymbolValidationRecord *other)
{
    ...
    return LexicalOrder::by(_method, rhs->_method)
        ...
        .thenBy(_appendixObjectNull, rhs->_appendixObjectNull)
        .less();
}
...
bool TR::HandleMethodFromCPIndex::isLessThanWithinKind(SymbolValidationRecord *other)
{
